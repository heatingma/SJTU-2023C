...
def problem_1():
    ...
    histogram(df, "qty_f_veg", [x for x in range(0, 30, 2)], "pics/problem1/fresh_vegtables.png", standard=[6, 30])
    histogram(df, "num_day_foods", [x for x in range(0, 20, 2)], "pics/problem1/num_day_foods.png", standard=[12, 20])
    histogram(df, "qty_f_fruits", [x for x in np.arange(0, 8, 0.5)], "pics/problem1/quantity_fresh_fruits.png", standard=[4, 7])
    histogram(df, "qty_d_prods", [x for x in np.arange(0, 8, 0.5)], "pics/problem1/dairy_products.png", standard=[4, 10])
    histogram(df, "qty_cereal", [x for x in np.arange(0, 5, 0.5)], "pics/problem1/quantity_cereal.png", standard=[1, 3])
    histogram(df, "qty_lpfem", [x for x in range(0, 15, 1)], "pics/problem1/quantity_lpfem.png", standard=[2.4, 4])
    histogram(df, "qty_oil", [x for x in range(0, 150, 10)], "pics/problem1/quantity_oil.png", standard=[25, 30])
    histogram(df, "qty_beans", [x for x in np.arange(0, 1.2, 0.05)], "pics/problem1/quantity_beans.png", standard=[0.6, 1])
    histogram(df, "qty_salt", [x for x in range(0, 30, 2)], "pics/problem1/quantity_salt.png", standard=[0, 5])
    histogram(df, "qty_wine", [x for x in np.arange(0, 50, 5)], "pics/problem1/quantity_wine.png", standard=[0, 15])
    histogram(df, "qty_beverage", [x for x in np.arange(0, 1.2, 0.05)], "pics/problem1/quantity_beverage.png", standard=[0, 1])
    histogram(df, "ratio_at_home", [x for x in np.arange(0, 1, 0.1)], "pics/problem1/ratio_at_home.png", standard=[0.5, 1])
...
def problem_2():
    ...
    cca(X2, Y, "pics/problem2/CCA_2.png", figsize=(12,6))
...
def problem_3():
    ...
    # cca
    cca(Y, X, "pics/problem3/CCA_3.png", figsize=(25, 12))
    # corr
